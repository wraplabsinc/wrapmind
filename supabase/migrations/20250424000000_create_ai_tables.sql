...
-- RLS: org members can read their own usage; service role can insert
...
-- ai_circuit_breaker: global failure counter
...
-- Seed global row
insert into ai_circuit_breaker (feature_key, failure_count, last_failure_at, circuit_open_until)
...
create policy "service_role_manage" on ai_circuit_breaker
  for all using (auth.role() = 'service_role');

-- Functions: global circuit breaker (no feature_key param)
...
begin
  ...
  if rec.failure_count >= 3 and rec.last_failure_at > now() - interval '5 minutes' then
    update ai_circuit_breaker
    set circuit_open_until = now() + interval '10 minutes'
    where feature_key = 'global';
    return true;
  end if;
