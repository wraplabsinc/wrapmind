...
BEGIN
    ...
    RAISE NOTICE 'Total: %. GC: % (%.1f%%) CW: % (%.1f%%) GW: % (%.1f%%)', 
        v_total, v_gc, CASE WHEN v_total > 0 THEN (v_gc * 100.0 / v_total) ELSE 0 END,
        v_cw, CASE WHEN v_total > 0 THEN (v_cw * 100.0 / v_total) ELSE 0 END,
        v_gw, CASE WHEN v_total > 0 THEN (v_gw * 100.0 / v_total) ELSE 0 END;
END $$;