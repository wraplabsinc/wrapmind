...
-- https://supabase.com/dashboard/project/nbewyeoiizlsfmbqoist/sql/new
...
CREATE TABLE IF NOT EXISTS sm_import_customers (
    ...
    sm_data JSONB, imported_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS sm_import_vehicles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID REFERENCES organizations(id) NOT NULL,
    sm_vehicle_id VARCHAR(100) NOT NULL, sm_customer_id VARCHAR(100),
    year INTEGER, make VARCHAR(100), model VARCHAR(100), vin VARCHAR(50),
    license_plate VARCHAR(50), color VARCHAR(50), vehicle_type VARCHAR(50),
    sm_data JSONB, imported_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS sm_import_orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID REFERENCES organizations(id) NOT NULL,
    sm_order_id VARCHAR(100) NOT NULL, sm_customer_id VARCHAR(100), sm_vehicle_id VARCHAR(100),
    status VARCHAR(100), total DECIMAL(10,2), tax_total DECIMAL(10,2),
    labor_total DECIMAL(10,2), parts_total DECIMAL(10,2), notes TEXT,
    sm_vehicle_data JSONB, sm_data JSONB,
...
CREATE TABLE IF NOT EXISTS sm_import_order_lines (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID REFERENCES organizations(id) NOT NULL,
    sm_order_id VARCHAR(100) NOT NULL, sm_line_id VARCHAR(100),
    description TEXT, line_type VARCHAR(50),
    quantity DECIMAL(10,2), unit_price DECIMAL(10,2), total DECIMAL(10,2), tax_rate DECIMAL(5,4),
    sm_data JSONB, imported_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS sm_import_labor_rates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id UUID REFERENCES organizations(id) NOT NULL,
    sm_labor_rate_id VARCHAR(100) NOT NULL, name VARCHAR(100), rate DECIMAL(10,2),
    sm_data JSONB, imported_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
