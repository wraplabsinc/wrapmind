-- Fix VIN year codes to handle 30-year cycle (VIN years repeat 1980+30=2010, 1981+30=2011, etc.)
DROP TABLE IF EXISTS vin_year_codes;

CREATE TABLE IF NOT EXISTS vin_year_codes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    year_char VARCHAR(1) NOT NULL UNIQUE,
    year INTEGER NOT NULL  -- Actual year (accounting for 30-year cycle)
);

-- 1980s and 2010s share same codes, 1990s and 2020s share same codes
...
-- Update the decode function to use actual year
...
BEGIN
    ...
    -- Look up year (most recent match for current vehicles)
    SELECT year INTO v_year FROM vin_year_codes WHERE year_char = v_year_char ORDER BY year DESC LIMIT 1;
