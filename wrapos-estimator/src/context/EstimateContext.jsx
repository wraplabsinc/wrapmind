...
import * as svc from '../lib/estimateService';

const STORAGE_KEY = 'wm-estimates-v1';
const MIGRATION_KEY = 'wm-estimates-migrated';
...
const SEED_ESTIMATES = [
  {
    ...
    vehicleVin: '5YJ3E1EA1PF123456',
    package: 'Full Wrap',
    material: '3M 1080 Series',
    materialColor: 'Matte Charcoal',
    laborHours: 18,
    basePrice: 2800,
    laborCost: 900,
    materialCost: 650,
  ...
  },
  {
    ...
    vehicleVin: 'WBS43AY01NCK98765',
    package: 'Full Wrap',
    material: 'Avery Dennison SW900',
    materialColor: 'Satin Black',
    laborHours: 20,
    basePrice: 3200,
    laborCost: 1000,
    materialCost: 720,
    ...
    sentAt: '2025-01-23T09:30:00Z',
    expiresAt: '2025-02-22T00:00:00Z',
    approvedAt: '2025-01-25T16:45:00Z',
    declinedAt: null,
  ...
  },
  {
    ...
    customerPhone: '(213) 555-0458',
    customerEmail: 'kyle.huang@email.com',
    vehicleId: 'v004',
    vehicleLabel: '2022 Porsche 911',
    vehicleVin: 'WP0AA2A99NS223344',
    package: 'Full Wrap',
    material: 'ORACAL 970RA',
    materialColor: 'Gloss Frozen Cherry',
    laborHours: 22,
    basePrice: 3600,
    laborCost: 1100,
    materialCost: 890,
    ...
    approvedAt: null,
    declinedAt: '2025-02-05T17:30:00Z',
    convertedToInvoice: false,
    invoiceId: null,
  },
  {
    ...
    customerPhone: '(323) 555-0572',
    customerEmail: 'jordan.lee@email.com',
    vehicleId: 'v005',
    vehicleLabel: '2020 Jeep Wrangler',
    vehicleVin: '1C4HJXDG0LW123789',
    package: 'Partial Wrap',
    material: '3M 1080 Series',
    materialColor: 'Matte Military Green',
    laborHours: 12,
    basePrice: 1800,
    laborCost: 600,
    materialCost: 480,
  ...
  },
  {
    ...
    customerPhone: '(714) 555-0614',
    customerEmail: 'priya.nair@email.com',
    vehicleId: 'v006',
    vehicleLabel: '2024 Honda CR-V',
    vehicleVin: '7FARW2H50RE012345',
    package: 'Full Wrap',
    material: 'Avery Dennison SW900',
    materialColor: 'Gloss Cosmic Blue',
    laborHours: 16,
    basePrice: 2600,
    laborCost: 800,
    materialCost: 610,
  ...
  },
  {
    ...
    customerPhone: '(562) 555-0771',
    customerEmail: 'brett.tanaka@fleetco.com',
    vehicleId: 'v007',
    vehicleLabel: '2023 Ford Transit (Fleet)',
    vehicleVin: '1FTBR3X85PKA11111',
    package: 'Full Wrap',
    material: '3M 1080 Series',
    materialColor: 'Gloss White + Logo Overlay',
    laborHours: 24,
    basePrice: 4200,
    laborCost: 1200,
    materialCost: 980,
  ...
  },
  {
    ...
    status: 'sent',
    customerId: 'c008',
    customerName: 'Destiny Monroe',
    customerPhone: '(626) 555-0832',
    customerEmail: 'destiny.monroe@email.com',
    vehicleId: 'v011',
    vehicleLabel: '2023 Audi RS5',
    vehicleVin: 'WUAPEWF58PA001122',
    package: 'Racing Stripes',
    material: 'ORACAL 651',
    materialColor: 'Gloss Neon Yellow',
    laborHours: 6,
    basePrice: 800,
    laborCost: 300,
    materialCost: 220,
  ...
  },
  {
    id: 'est-012',
    locationId: 'loc-002',
    estimateNumber: 'WM-0012',
    status: 'draft',
    customerId: 'c009',
    customerName: 'Sam Okafor',
    customerPhone: '(415) 555-0991',
    customerEmail: 'sam.okafor@email.com',
    vehicleId: 'v012',
    vehicleLabel: '2024 Dodge Challenger',
    vehicleVin: '2C3CDZBT5RH123456',
    package: 'Partial Wrap',
    material: 'Avery Dennison SW900',
    materialColor: 'Matte Carbon Fiber',
    laborHours: 14,
    basePrice: 2100,
    laborCost: 700,
    materialCost: 560,
...
export function EstimateProvider({ children }) {
  ...
  const [estimates, setEstimates] = useState(() => {
    if (import.meta.env.VITE_DEV_AUTH === '1') return SEED_ESTIMATES;
  ...
  useEffect(() => { saveToStorage(estimates); }, [estimates]);
    ...
    (async () => {
      try {
        ...
        if (!migrated) {
          ...
          if (local.length > 0) {
            await svc.migrateLocalEstimates(local, orgId);
          }
          localStorage.setItem(MIGRATION_KEY, orgId);
        }

        const remote = await svc.fetchEstimates(orgId);
        if (!cancelled) {
          setEstimates(remote);
        }
      } catch (err) {
        console.error('[EstimateContext] Supabase sync failed, using localStorage:', err);
      }
    })();
  ...
  // ─── Filtered view ─────────────────────────────────────────────────────────
  const filteredEstimates = activeLocationId === 'all'
    ? estimates
    : estimates.filter(e => !e.locationId || e.locationId === activeLocationId);

  // ─── getNextEstimateNumber (org-scoped via synced state) ───────────────────
  ...
  const addEstimate = useCallback((estimateData = {}) => {
    ...
    if (orgId) {
      svc.insertEstimate(newEst, orgId).catch(err =>
        console.error('[EstimateContext] Supabase insert failed:', err)
      );
    }

    return newEst;
  }, [estimates, activeLocationId, orgId, org]);
  ...
  const updateEstimate = useCallback((id, patch) => {
    ...
    if (orgId) {
      svc.patchEstimate(id, patch).catch(err =>
        console.error('[EstimateContext] Supabase update failed:', err)
      );
    }
  }, [orgId]);
  ...
  const deleteEstimate = useCallback((id) => {
    ...
    if (orgId) {
      svc.removeEstimate(id).catch(err =>
        console.error('[EstimateContext] Supabase delete failed:', err)
      );
    }
  }, [orgId]);
