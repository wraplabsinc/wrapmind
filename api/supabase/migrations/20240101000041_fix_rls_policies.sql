...
DROP POLICY IF EXISTS "API key access to cars" ON cars;
...
DROP POLICY IF EXISTS "API key access to wrap_packages" ON wrap_packages;
CREATE POLICY "Allow all access wrap_packages" ON wrap_packages
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "API key access to wrap_materials" ON wrap_materials;
CREATE POLICY "Allow all access wrap_materials" ON wrap_materials
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "API key access to dimension_references" ON dimension_references;
