// =============================================================================
// InvoiceArchiveDrawer — Archive history drawer for InvoiceDetailPanel
...
export default function InvoiceArchiveDrawer({ invoiceId, invoiceDocNumber, onClose }) {
  ...
  useEffect(() => {
    if (!invoiceId || !session?.access_token) return;
    let cancelled = false;
    setLoading(true);
    setError('');
    ...
    fetch(`${SUPABASE_URL}/functions/v1/get-archive`, {
      ...
      body: JSON.stringify({ docType: 'invoice', docId: invoiceId }),
    })
      .then(r => r.json())
      .then(data => {
        if (cancelled) return;
  ...
  }, [invoiceId, session?.access_token]);
    ...
    try {
      await fetch(`${SUPABASE_URL}/functions/v1/send-email`, {
        ...
        body: JSON.stringify({
          ...
          subject: `Invoice ${invoiceDocNumber}`,
          body: `Please find your invoice at the following link:\n\n${archive.storageUrl}\n\nDocument: ${invoiceDocNumber}`,
          documentType: 'invoice',
          documentId: invoiceId,
        }),
      });
    } catch (err) {
      console.error('[InvoiceArchiveDrawer] resend failed:', err);
    } finally {
      setResending(null);
    }
  }
          ...
          {!loading && !error && archives.length === 0 && (
            <div className="flex flex-col items-center justify-center h-40 text-center px-6">
              ...
              <p className="text-xs text-[#64748B] dark:text-[#7D93AE] mt-1">
                PDFs generated for this invoice will appear here
              </p>
            </div>
          )}
